...
namespace Microsoft.Teams.Apps.FAQPlusPlus.Common.Providers
{
    ...
    public class ConfigurationDataProvider : IConfigurationDataProvider
    {
        ...
        /// <summary>
        /// Partition key of Configuration info table.
        /// </summary>
        ...
        /// <returns>A <see cref="Task"/> of type bool where true represents updated data is saved or updated successfully while false indicates failure in saving or updating the updated data.</returns>
        public async Task<bool> UpsertLanguageKBConfigurationEntityAsync(string languageCode, LanguageKBConfigurationEntity languageKBConfiguration)
        {
            var tableResult = await this.StoreOrUpdateLanguageKBConfigurationEntityAsync(languageKBConfiguration).ConfigureAwait(false);
            return tableResult.HttpStatusCode == (int)HttpStatusCode.NoContent;
        }
        ...
        /// <param name="languageCode">Language code from view based on which appropriate row will replaced or inserted in table storage.</param>
        /// <returns><see cref="Task"/>Already saved entity detail.</returns>
        public async Task<LanguageKBConfigurationEntity> GetSavedLanguageKBConfigurationEntityAsync(string languageCode)
        {
            await this.EnsureInitializedAsync().ConfigureAwait(false);
            var searchOperation = TableOperation.Retrieve<LanguageKBConfigurationEntity>(Constants.LanguageKBConfigurationPartitionKey, languageCode);
            TableResult searchResult = await this.configurationCloudTable.ExecuteAsync(searchOperation).ConfigureAwait(false);
            var result = (LanguageKBConfigurationEntity)searchResult.Result;
            return result;
        }
        ...
        public async Task<List<LanguageKBConfigurationEntity>> GetAllLanguageKBConfigurationEntitiesAsync()
        {
            await this.EnsureInitializedAsync().ConfigureAwait(false);
            var query = new TableQuery<LanguageKBConfigurationEntity>().Where(TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, Constants.LanguageKBConfigurationPartitionKey));
            TableContinuationToken continuationToken = null;
            var languageKBConfigurationEntityList = new List<LanguageKBConfigurationEntity>();
            do
            {
                var querySegment = await this.configurationCloudTable.ExecuteQuerySegmentedAsync<LanguageKBConfigurationEntity>(query, continuationToken);
                languageKBConfigurationEntityList.AddRange(querySegment.Results);
                continuationToken = querySegment.ContinuationToken;
            }
            while (continuationToken != null);

            return languageKBConfigurationEntityList;
        }
        ...
        /// Store or update configuration entity in table storage.
        /// </summary>
        /// <param name="entity">Language knowledgebase configuration entity object.</param>
        /// <returns><see cref="Task"/> That represents configuration entity is saved or updated.</returns>
        private async Task<TableResult> StoreOrUpdateLanguageKBConfigurationEntityAsync(LanguageKBConfigurationEntity entity)
        {
            await this.EnsureInitializedAsync().ConfigureAwait(false);
            TableOperation addOrUpdateOperation = TableOperation.InsertOrReplace(entity);
            return await this.configurationCloudTable.ExecuteAsync(addOrUpdateOperation).ConfigureAwait(false);

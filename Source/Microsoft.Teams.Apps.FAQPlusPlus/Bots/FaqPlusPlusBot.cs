...
namespace Microsoft.Teams.Apps.FAQPlusPlus.Bots
{
    ...
    public class FaqPlusPlusBot : TeamsActivityHandler
    {
        ...
        /// <summary>
        /// Represents the task module height for normal card.
        /// </summary>
        private const int TaskModuleNormalCardHeight = 250;
        ...
        private readonly IUserLanguagePreferenceProvider userLanguagePreferenceProvider;
		...
		/// <param name="userLanguagePreferenceProvider">User language preference provider.</param>
        /// <param name="configurationSettings">Configuration setting instance to get the appsettings.</param>
        public FaqPlusPlusBot(
            ...
            IEnumerable<IQnaServiceProvider> qnaServiceProviders,
			...
			IUserLanguagePreferenceProvider userLanguagePreferenceProvider,
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                var currentOrDefaultLanguageCode = await this.GetCurrentOrDefaultLanguageCodeForTurnContext(turnContext);
                if (currentOrDefaultLanguageCode != string.Empty)
                {
                    CultureInfo.CurrentCulture = CultureInfo.CurrentUICulture = CultureInfo.GetCultureInfo(currentOrDefaultLanguageCode);
                }
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(postedQuestionData.UpdatedQuestion?.Trim())
                       && !string.IsNullOrEmpty(postedQuestionData.Description?.Trim())
                       && string.IsNullOrEmpty(postedQuestionData.ImageUrl?.Trim()))
                    {
                        // Preview the actual view of the card on preview button click.
                        return await GetTaskModuleNormalCardResponseAsync(MessagingExtensionQnaCard.PreviewCardResponse(postedQuestionData, this.appBaseUri)).ConfigureAwait(false);
                    }
                    else
                    {
                        // Preview the actual view of the card on preview button click.
            ...
            catch (Exception ex)
            {
                ...
                if (((ErrorResponseException)ex?.InnerException)?.Response.StatusCode == HttpStatusCode.BadRequest)
                {
                    ...
                    var applicableQnaServiceProvider = await this.GetApplicableQnaServiceProvider(turnContext);
                    var hasPublished = await applicableQnaServiceProvider.GetInitialPublishedStatusAsync(knowledgeBaseId).ConfigureAwait(false);
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var allLanguageKBConfigurations = await this.configurationProvider.GetAllLanguageKBConfigurationEntitiesAsync().ConfigureAwait(false);
                var currentTeamConfiguration = allLanguageKBConfigurations.FirstOrDefault(lkb => lkb.TeamId.Equals(teamsChannelData?.Team?.Id, StringComparison.InvariantCultureIgnoreCase));

                if (turnContext != null && currentTeamConfiguration != null && await this.IsMemberOfSmeTeamAsync(currentTeamConfiguration.TeamId, turnContext).ConfigureAwait(false))
                {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                var allLanguageKbConfigurations = this.configurationProvider.GetAllLanguageKBConfigurationEntitiesAsync().GetAwaiter().GetResult();
                var currentLanguageLbConfiguration = allLanguageKbConfigurations.FirstOrDefault(lkb => lkb.TeamId == teamsChannelData?.Team?.Id);

                if (currentLanguageLbConfiguration == null)
                {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(postedQuestionObject.UpdatedQuestion?.Trim())
                       && !string.IsNullOrEmpty(postedQuestionObject.Description?.Trim())
                       && string.IsNullOrEmpty(postedQuestionObject.ImageUrl?.Trim()))
                    {
                        return await GetMessagingExtensionActionNormalCardResponseAsync(MessagingExtensionQnaCard.PreviewCardResponse(postedQuestionObject, this.appBaseUri)).ConfigureAwait(false);
                    }
                    else
                    {
                        // Preview the actual view of the card on preview button click.
        ...
        /// <param name="titleText">Gets or sets text that appears below the app name and to the right of the app icon.</param>
        /// <returns>Envelope for Task Module Response.</returns>
        private static Task<TaskModuleResponse> GetTaskModuleNormalCardResponseAsync(Attachment questionAnswerAdaptiveCardEditor, string titleText = "")
        {
            return Task.FromResult(new TaskModuleResponse
            {
                Task = new TaskModuleContinueResponse
                {
                    ...
                    {
                        Card = questionAnswerAdaptiveCardEditor,
                        Height = TaskModuleNormalCardHeight,
                        Width = TaskModuleWidth,
                        Title = titleText,
                    },
                },
        ...
        /// <param name="titleText">Gets or sets text that appears below the app name and to the right of the app icon.</param>
        /// <returns>Response of messaging extension action object.</returns>
        private static Task<MessagingExtensionActionResponse> GetMessagingExtensionActionNormalCardResponseAsync(
            Attachment questionAnswerAdaptiveCardEditor,
            string titleText = "")
        {
            return Task.FromResult(new MessagingExtensionActionResponse
        ...
        CancellationToken cancellationToken)
        {
            ...
            // Get applicable QnaServiceProvider for current applicable language.
            var applicableQnaServiceProvider = await this.GetApplicableQnaServiceProvider(turnContext);
            var allLanguageKBCombinations = await this.configurationProvider.GetAllLanguageKBConfigurationEntitiesAsync();
            var currentLanguageKBCombination = allLanguageKBCombinations.FirstOrDefault(lkb => lkb.RowKey.Equals(applicableQnaServiceProvider.GetApplicableLanguageCode(), StringComparison.InvariantCultureIgnoreCase));
            string knowledgeBaseId = currentLanguageKBCombination.KnowledgeBaseId;

            try
            {
                // Check if question exist in the production/test knowledgebase & exactly the same question.
                var hasQuestionExist = await applicableQnaServiceProvider.QuestionExistsInKbAsync(qnaPairEntity.UpdatedQuestion).ConfigureAwait(false);
            ...
            await applicableQnaServiceProvider.AddQnaAsync(knowledgeBaseId, qnaPairEntity.UpdatedQuestion?.Trim(), combinedDescription, turnContext.Activity.From.AadObjectId, turnContext.Activity.Conversation.Id, activityReferenceId).ConfigureAwait(false);
            ...
            string answer)
        {
            ...
            else
            {
                ...
                // Check if question exist in the production/test knowledgebase & exactly the same question.
                var hasQuestionExist = await applicableQnaServiceProvider.QuestionExistsInKbAsync(postedQnaPairEntity.UpdatedQuestion).ConfigureAwait(false);
            ...
            ITurnContext<IConversationUpdateActivity> turnContext)
        {
            ...
            {
                var allLanguagesSupportedByBot = await this.GetAllLanguagesSupportedByBot();
                var defaultLanguageCode = allLanguagesSupportedByBot.FirstOrDefault(l => l.Default).Code;
                var allLanguageKBCombinations = await this.configurationProvider.GetAllLanguageKBConfigurationEntitiesAsync();
                var currentLanguageKBCombination = allLanguageKBCombinations.FirstOrDefault(lkb => lkb.RowKey.Equals(defaultLanguageCode, StringComparison.InvariantCultureIgnoreCase));
                ...
                var userWelcomeCardAttachment = WelcomeCard.GetCard(currentLanguageKBCombination.ChangeLanguageMessageText);
        ...
        private async Task OnMessageActivityInPersonalChatAsync(
            ...
            CancellationToken cancellationToken)
        {
            var botSupportedLanguages = await this.GetAllLanguagesSupportedByBot();
            string botLanguageCode = await this.GetCurrentOrDefaultLanguageCodeForTurnContext(turnContext);

            if (botLanguageCode != string.Empty)
            {
                CultureInfo.CurrentCulture = CultureInfo.CurrentUICulture = CultureInfo.GetCultureInfo(botLanguageCode);
            }
            ...
            {
                if (text.Equals(Constants.ChangeLanguage))
                {
                    this.logger.LogInformation("Sending change language card to the user");
                    await turnContext.SendActivityAsync(MessageFactory.Attachment(ChangeLanguageCard.GetCard(botSupportedLanguages))).ConfigureAwait(false);
                }
                else if (text.Equals(Strings.AskAnExpertCommandText.ToLower()))
                {
                ...
                else if (text.Equals(Strings.ShareFeedbackCommandText.ToLower()))
                {
                ...
                else if (text.Equals(Strings.TakeATourButtonText.ToLower()))
                {
                ...
                else if (text.Equals(Strings.MyQuestionLabelText.ToLower()))
                {
                    this.logger.LogInformation("Sending Redirect to My Question tab card");
                    await turnContext.SendActivityAsync(MessageFactory.Attachment(new GoToMyQuestionTabCard(this.configurationSettings).GetCard())).ConfigureAwait(false);
                }
                else
                {
            ...
            try
            {
                switch (text)
                {
                    ...
                    case Constants.DeleteCommand:
                        // Get the Language Knowledgebase configuration entity object for the current applicable language code.
                        var languageKBConfigurationEntity = await this.configurationProvider.GetSavedLanguageKBConfigurationEntityAsync(applicableQnaServiceProvider.GetApplicableLanguageCode());

                        this.logger.LogInformation($"Delete card submit in channel {message.Value?.ToString()}");
                        await QnaHelper.DeleteQnaPair(languageKBConfigurationEntity.KnowledgeBaseId, turnContext, applicableQnaServiceProvider, this.activityStorageProvider, this.logger, cancellationToken).ConfigureAwait(false);
                        break;
            ...
            CancellationToken cancellationToken)
        {
            ...
            var payload = ((JObject)message.Value).ToObject<AdaptiveSubmitActionData>();

            string currentUserLanguageCode = await this.GetCurrentOrDefaultLanguageCodeForTurnContext(turnContext);

            // Check if this submit action corrsponds to change language card.
            if (payload.LanguageSelected != null)
            {
                this.logger.LogInformation("Sending welcome message for selected language to the user");
                try
                {
                    var userLanguagePreferenceEntity = new UserLanguagePreferenceEntity() { UserObjectId = message.From.AadObjectId, LanguageCode = payload.LanguageSelected };
                    await this.userLanguagePreferenceProvider.UpsertLanguagePreferenceAsync(userLanguagePreferenceEntity);
                    var selectedLanguageKBConfiguration = await this.configurationProvider.GetSavedLanguageKBConfigurationEntityAsync(payload.LanguageSelected);
                    await turnContext.SendActivityAsync(MessageFactory.Attachment(LanguageSelectionCard.GetCard(selectedLanguageKBConfiguration.ChangeLanguageMessageText))).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
                    this.logger.LogError(ex, "Error while selecting default conversation language for the user.");
                }
            }
            else
            {
                if (message != null)
                {
                    if (message.Text.ToLower().Equals(Strings.AskAnExpertCommandText.ToLower()))
                    {
                    ...
                    else if (message.Text.ToLower().Equals(Strings.ShareFeedbackCommandText.ToLower()))
                    {
                    ...
                    else if (message.Text.Equals(AskAnExpertCard.AskAnExpertSubmitText))
                    {
                        this.logger.LogInformation("Received question for expert");
                        newTicket = await AdaptiveCardHelper.AskAnExpertSubmitText(currentUserLanguageCode, message, turnContext, cancellationToken, this.ticketsProvider).ConfigureAwait(false);
                        if (newTicket != null)
                        {
                            smeTeamCard = new SmeTicketCard(newTicket, null).ToAttachment(message?.LocalTimestamp, this.appBaseUri, false);
                            userCard = new UserNotificationCard(newTicket, this.configurationSettings).ToAttachment(Strings.NotificationCardContent, message?.LocalTimestamp);
                        }
                    }
                    else if (message.Text.Equals(ShareFeedbackCard.ShareFeedbackSubmitText))
                    {
            ...
            var languageKBConfiguration = await this.configurationProvider.GetSavedLanguageKBConfigurationEntityAsync(currentUserLanguageCode);

            // Send message to SME team.
            if (smeTeamCard != null)
            {
                var resourceResponse = await this.SendCardToTeamAsync(turnContext, smeTeamCard, languageKBConfiguration.TeamId, cancellationToken).ConfigureAwait(false);
        ...
        private async Task OnAdaptiveCardSubmitInChannelAsync(
            ...
            CancellationToken cancellationToken)
        {
            var payload = ((JObject)message.Value).ToObject<UpdateTicketResponsePayload>();
            this.logger.LogInformation($"Received submit: ticketId={payload.TicketId} action={payload.Action}");
            var userDetails = await AdaptiveCardHelper.GetUserDetailsInChannelChatAsync(turnContext, cancellationToken, message.Id, message.From.Name).ConfigureAwait(false);
            ...
            if (payload.Action == UpdateTicketResponsePayload.RespondAction && string.IsNullOrWhiteSpace(payload.AnswerForRespond))
            {
                // Update the card in the SME team to show validation.
                var updatevalidationCardActivity = new Activity(ActivityTypes.Message)
                {
                    Id = ticket.SmeCardActivityId,
                    Conversation = new ConversationAccount { Id = ticket.SmeThreadConversationId },
                    Attachments = new List<Attachment> { new SmeTicketCard(ticket, payload).ToAttachment(message.LocalTimestamp, this.appBaseUri, true) },
                };
                var updatevalidationResponse = await turnContext.UpdateActivityAsync(updatevalidationCardActivity, cancellationToken).ConfigureAwait(false);
                return;
            }
            else if (payload.Action != UpdateTicketResponsePayload.RespondAction && string.IsNullOrWhiteSpace(payload.Answer))
            {
                // Update the card in the SME team to show validation.
                var updatevalidationCardActivity = new Activity(ActivityTypes.Message)
                {
            ...
            {
                case UpdateTicketResponsePayload.RespondAction:
                    ticket.Status = (int)TicketState.Answered;
                    ...
                    ticket.AssignedToUserPrincipalName = userDetails.UserPrincipalName;
                    ticket.AnswerBySME = payload.AnswerForRespond;
                    if (payload.AddorAppendAction == Strings.AppendActionMessage)
                    {
                        ticket.KnowledgeBaseAnswer = ticket.KnowledgeBaseAnswer + " " + payload.AnswerForRespond;

                        // Update the question in the knowledgebase.
                        QnASearchResult qnasearchResult;
                        var qnaKBAnswerResponse = await applicableQnaServiceProvider.GenerateAnswerAsync(ticket.KnowledgeBaseQuestion, false).ConfigureAwait(false);
                        qnasearchResult = qnaKBAnswerResponse.Answers.FirstOrDefault();
                        bool isSameKBQuestion = false;

                        // Check if question exist in the knowledgebase.
                        if (qnasearchResult != null && qnasearchResult.Questions.Count > 0)
                        {
                            // Check if the edited question & result returned from the knowledgebase are same.
                            isSameKBQuestion = qnasearchResult.Questions.First().ToUpperInvariant() == ticket.KnowledgeBaseQuestion.ToUpperInvariant();
                        }
                        ...
                        if (qnasearchResult.Id != -1 && isSameKBQuestion)
                        {
                            int qnaPairId = qnasearchResult.Id.Value;
                            await applicableQnaServiceProvider.UpdateQnaAsync(knowledgeBaseId, qnaPairId, ticket.KnowledgeBaseAnswer, turnContext.Activity.From.AadObjectId, ticket.KnowledgeBaseQuestion, ticket.KnowledgeBaseQuestion).ConfigureAwait(false);
                ...
                case UpdateTicketResponsePayload.AddRespondAction:
                    ticket.Status = (int)TicketState.Answered;
                    ticket.DateAssigned = DateTime.UtcNow;
                    ticket.AssignedToName = message.From.Name;
                    ticket.AssignedToObjectId = message.From.AadObjectId;
                    ticket.AssignedToUserPrincipalName = userDetails.UserPrincipalName;
                    ticket.AnswerBySME = payload.Answer;
                    if (payload.AddorAppendAction == Strings.AddActionMessage)
                    {
                        ticket.KnowledgeBaseAnswer = payload.Answer;

                        // Save the question in the knowledgebase.
                        var activityReferenceId = Guid.NewGuid().ToString();
                        await applicableQnaServiceProvider.AddQnaAsync(knowledgeBaseId, ticket.Title.Trim(), payload.Answer, turnContext.Activity.From.AadObjectId, turnContext.Activity.Conversation.Id, activityReferenceId).ConfigureAwait(false);
                        this.logger.LogInformation($"Ticket {payload.TicketId} was added to KB with referenceId:{activityReferenceId}");
                        this.logger.LogInformation($"Question added by: {turnContext.Activity.From.AadObjectId}");
                    }

                    break;

                case UpdateTicketResponsePayload.UpdateResponseAction:
                    ticket.Status = (int)TicketState.Answered;
                    ticket.DateAssigned = DateTime.UtcNow;
                    ticket.AssignedToName = message.From.Name;
                    ticket.AssignedToObjectId = message.From.AadObjectId;
                    ...
                    var qnaAnswerResponse = await applicableQnaServiceProvider.GenerateAnswerAsync(ticket.KnowledgeBaseQuestion, false).ConfigureAwait(false);
                    ...
                    {
                        // Check if the edited question & result returned from the knowledgebase are same.
                        isSameQuestion = searchResult.Questions.First().ToUpperInvariant() == ticket.KnowledgeBaseQuestion.ToUpperInvariant();
                    }
                    ...
                    if (searchResult.Id != -1 && isSameQuestion)
                    {
                        ...
                        await applicableQnaServiceProvider.UpdateQnaAsync(knowledgeBaseId, qnaPairId, payload.Answer, turnContext.Activity.From.AadObjectId, ticket.KnowledgeBaseQuestion, ticket.KnowledgeBaseQuestion).ConfigureAwait(false);
                        this.logger.LogInformation($"Question updated by: {turnContext.Activity.Conversation.AadObjectId}");
                    }
            ...
            var updateCardActivity = new Activity(ActivityTypes.Message)
            {
                Id = ticket.SmeCardActivityId,
                Conversation = new ConversationAccount { Id = ticket.SmeThreadConversationId },
                Attachments = new List<Attachment> { new SmeTicketCard(ticket, payload).ToAttachment(message.LocalTimestamp, this.appBaseUri, false) },
            };
            ...
            {
                case UpdateTicketResponsePayload.RespondAction:
                    smeNotification = string.Format(CultureInfo.InvariantCulture, Strings.SMEAnsweredResponseStatus, message.From.Name);

                    userNotification = MessageFactory.Attachment(new UserNotificationCard(ticket, this.configurationSettings).ToAttachment(Strings.SMEReplyCardHeaderText, message.LocalTimestamp));
                    userNotification.Summary = Strings.AnsweredTicketUserNotification;
                    break;
                ...
                case UpdateTicketResponsePayload.UpdateResponseAction:
                    smeNotification = string.Format(CultureInfo.InvariantCulture, Strings.SMEAnsweredUpdateStatus, message.From.Name);
        ...
        private async Task<bool> SaveQnaAsync(ITurnContext turnContext, string answer, AdaptiveSubmitActionData qnaPairEntity)
        {
            ...
            var applicableQnaServiceProvider = await this.GetApplicableQnaServiceProvider(turnContext);
            QnASearchResult searchResult;
            var qnaAnswerResponse = await applicableQnaServiceProvider.GenerateAnswerAsync(qnaPairEntity.OriginalQuestion, qnaPairEntity.IsTestKnowledgeBase).ConfigureAwait(false);
            searchResult = qnaAnswerResponse.Answers.FirstOrDefault();
            bool isSameQuestion = false;
            ...
            if (searchResult.Id != -1 && isSameQuestion)
            {
                ...
                var languageKBConfigurationEntity = await this.configurationProvider.GetSavedLanguageKBConfigurationEntityAsync(applicableQnaServiceProvider.GetApplicableLanguageCode());
                int qnaPairId = searchResult.Id.Value;
                await applicableQnaServiceProvider.UpdateQnaAsync(languageKBConfigurationEntity.KnowledgeBaseId, qnaPairId, answer, turnContext.Activity.From.AadObjectId, qnaPairEntity.UpdatedQuestion, qnaPairEntity.OriginalQuestion).ConfigureAwait(false);
                this.logger.LogInformation($"Question updated by: {turnContext.Activity.Conversation.AadObjectId}");
        ...
        /// </summary>
        /// <param name="currentExpertTeamId">Current team Id.</param>
        /// <param name="turnContext">Context object containing information cached for a single turn of conversation with a user.</param>
        ...
        private async Task<bool> IsMemberOfSmeTeamAsync(string currentExpertTeamId, ITurnContext<IInvokeActivity> turnContext)
        {
            ...
            {
                this.logger.LogError(error, $"Failed to get members of team {currentExpertTeamId}: {error.Message}", SeverityLevel.Error);
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (Validators.IsRichCard(postedQnaPairEntity))
                {
                    ...
                    if (Validators.IsImageUrlInvalid(postedQnaPairEntity))
                    {
            ...
            {
                this.logger.LogError(error, $"Failed in RespondToQuestionMessagingExtensionAsync: {error.Message}", SeverityLevel.Error);
                throw;
            }
        }
            ...
            ITurnContext<IInvokeActivity> turnContext)
        {
            ...
            {
                this.logger.LogError(error, $"Failed in RespondToQuestionTaskModuleAsync: {error.Message}", SeverityLevel.Error);
                throw;
            }
        }
            ...
            string text)
        {
            ...
            try
            {
                var queryResult = await applicableQnaServiceProvider.GenerateAnswerAsync(question: text, isTestKnowledgeBase: false).ConfigureAwait(false);
                ...
                if (queryResult.Answers.First().Id != -1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            await turnContext.SendActivityAsync(MessageFactory.Attachment(ResponseCard.GetCardWithPrompts(answerData.Questions.FirstOrDefault(), answerData.Answer, text, answerData.Context.Prompts, (int)answerData.Id))).ConfigureAwait(false);
                        }
                        else
                        {
                            await turnContext.SendActivityAsync(MessageFactory.Attachment(ResponseCard.GetCard(answerData.Answer, text, answerData.Questions[0]))).ConfigureAwait(false);
                        }
                    }
                }
                else
        ...
        private async Task<IQnaServiceProvider> GetApplicableQnaServiceProvider(ITurnContext turnContext)
        {
            string currentOrDefaultLanguageCode = await this.GetCurrentOrDefaultLanguageCodeForTurnContext(turnContext);
            IQnaServiceProvider applicableQnaServiceProvider = this.qnaServiceProviders.FirstOrDefault(qsp => qsp.GetApplicableLanguageCode().Equals(currentOrDefaultLanguageCode));

            return applicableQnaServiceProvider;
        }

        private async Task<string> GetCurrentOrDefaultLanguageCodeForTurnContext(ITurnContext turnContext)
        {
            string currentApplicableLanguageCode = string.Empty;
            ...
            {
                case ConversationTypePersonal:
                    var userLanguagePreference = await this.userLanguagePreferenceProvider.GetLanguagePreferenceAsync(activity.From.AadObjectId);
                    if (userLanguagePreference != null)
                    {
                        currentApplicableLanguageCode = userLanguagePreference.LanguageCode;
                    }
                ...
                case ConversationTypeChannel:
                    ...
                    // Get all language kb configurations from table storage.
                    var allLanguageKBCombinations = await this.configurationProvider.GetAllLanguageKBConfigurationEntitiesAsync();

                    // Find out the language kb configuration for the team to which the current turnContext corresponds to.
                    var applicableLanguageKBConfigurationForTeam = allLanguageKBCombinations.FirstOrDefault(l => l.TeamId.Equals(teamDetails.Team.Id, StringComparison.InvariantCultureIgnoreCase));
                    if (applicableLanguageKBConfigurationForTeam != null)
                    {
                        currentApplicableLanguageCode = applicableLanguageKBConfigurationForTeam.RowKey;
                    }

                    break;
            }

            // If applicableQnaServiceProvider is null, retrieve qnaServiceProvider for default language.
            if (currentApplicableLanguageCode == string.Empty)
            {
                // Get bot supported languages from table storage.
                ...
                var supportedLanguages = JsonConvert.DeserializeObject<List<BotLanguageDetail>>(supportedLanguagesJson);

                var defaultLanguage = supportedLanguages.FirstOrDefault(l => l.Default);
                currentApplicableLanguageCode = defaultLanguage.Code;
            }

            return currentApplicableLanguageCode;
        }

        /// <summary>
        /// Returns all languages supported by the bot.
        /// </summary>
        /// <returns>List of bot supported language detail objects.</returns>
        private async Task<List<BotLanguageDetail>> GetAllLanguagesSupportedByBot()
        {
            // Get details of languages supported by bot from the table storage.
            var supportedLanguagesJson = await this.configurationProvider.GetSavedEntityDetailAsync(ConfigurationEntityTypes.SupportedLanguagesKey).ConfigureAwait(false);
